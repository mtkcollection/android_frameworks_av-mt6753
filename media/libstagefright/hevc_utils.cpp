...
void parseProfileTierLevel(ABitReader *br, unsigned subLayers) {
    ...
    unsigned general_level_idc __unused = br->getBits(8);

    unsigned profilePresent[8];
    unsigned levelPresent[8];
    for (int i = 0; (unsigned)i < subLayers; i++) {
...
sp<MetaData> MakeHEVCMetaData(const sp<ABuffer> &accessUnit) {
    ...
    unsigned ppsLen __unused = picParamSet->size();
    if(seqParamSet != NULL){
    ...
    uint8_t* pps __unused = picParamSet->data();
#if 1
