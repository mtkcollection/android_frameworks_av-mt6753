...
static bool get_mp3_frame_size(uint32_t header, size_t *frame_size, unsigned *out_sampling_rate = NULL,
    ...
    *frame_size = 0;
    ...
    if (layer == 3) {
        ...
        static const unsigned kBitrateV1[] = {
        ...
        unsigned bitrate = (version == 3 /* V1 */) ?
                kBitrateV1[bitrate_index - 1] : kBitrateV2[bitrate_index - 1];
...
ASFSource::ASFSource(const sp<ASFExtractor> &extractor, size_t index)
    ...
    if (!strncasecmp(mime, "video/", 6)) {
        ALOGI(" MEDIA_MIMETYPE_VIDEO_RAW:mType = ASF_VIDEO");
        mType = ASF_VIDEO;
...
ASFSource::~ASFSource() {
    ...
    if (trackInfo->mCodecSpecificData!=NULL) {
        ALOGI("~ASFSource:free mCodecSpecificData=0x%p\n", trackInfo->mCodecSpecificData);
    ...
    while(0 == *pHeader) {
        ...
        while(start+ 3 < length) {
            ...
            if (get_mp3_frame_size(header1, &frame_size, NULL, NULL, NULL)) {
                ALOGI("possible header %u size %zu", header1, frame_size);
                ...
                if (j < 4) {
                    ...
                    ALOGD("[ASF_ERROR]ASFSource::findMP3Header End of this packet(= %d th, left= %d), read next.",
                         readPackets, left);
                    if (ASF_SUCCESS == mExtractor->GetNextMediaFrame(&packet2,
                        ...
                        if(packet2->range_length() < (size_t)left) {
                            ALOGD("ASF The packet(= %zu < left= %d)is too small to check MP3 Header.",
                        ...
                        ALOGD("[ASF_ERROR]ASFSource::findMP3Header End of stream,fake header = 0x%u", header1);
                        *pHeader = (int32_t)header1;
                        mExtractor->ASFSeekTo(0);
                        return OK;
                    }
                ...
                ALOGI("possible header %u size %zu, test %x", header1, frame_size, header2);
...
status_t ASFSource::start(MetaData *params) {
    ...
    if (params && params->findInt32(kKeyWantsNALFragments, &val) && val != 0) {
        ...
        mWantsNALFragments = false;
        ALOGI("[ASF]ASFSource::start mWantsNALFragments = false");
    }
...
status_t ASFSource::read(MediaBuffer **out, const ReadOptions *options) {
    ALOGI("ASFSource::read, mType=%d", mType);
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {//seek and find an I
        ALOGI("ASFSource::read seek to %lld us (mType=%d, mode=%d,mHasVideo=%d)",
...
RESET_SEEK_TIME:
        ...
        if (mExtractor->mHasVideo) {//has video file
            if (mType == ASF_VIDEO) {
                ...
                if (ret < 0) {
                    ALOGI("ASF Seek error");
                    //when seek error, if lastSeekTime is valid, then seek to lastSeekTime, otherwise return END_OF_FILE
                    if (lastSeekTime != -1) {
                        mExtractor->ASFSeekTo(lastSeekTime);
                        repeat_cnt = 10;
                        ALOGI("seek to last seek Time:%lld", (long long)lastSeekTime);
                    } else {
                        return ERROR_END_OF_STREAM;
                    }
                }
                ...
                while (!bIsKeyFrame) {
                    ...
                    ALOGI("GetNextMediaFrame when seek, return %d", retVal);
                    if ((ASF_END_OF_FILE == retVal) || (ASF_ERROR_MALFORMED == retVal)) {
                        ...
                        if (lastSeekTime != -1 && eosTimes < 2) {
                            ...
                            bIsKeyFrame = false;
                            repeat_cnt = 10;
                            ALOGI("seek to last seek Time %lld when ASF_END_OF_FILE, eosTimes=%u", (long long)lastSeekTime, eosTimes);
                        } else {
                            ALOGI("return EOS, eosTimes=%u", eosTimes);
                        ...
                        ALOGI("[ASF_ERROR]ASF_FILE_READ_ERR:ASF Video Seek Drop ts = %lld ms ", (long long)(keyTimeUs/1000));
                        (*out)->release();
                        (*out) = NULL;
                        bIsKeyFrame=false;
                        mSeeking=true;
                        ...
                        if( repeat_cnt < 10) {
                            if (((mode == ReadOptions::SEEK_PREVIOUS_SYNC && (diff > 0 || diff < -500000))) ||
                                ...
                                if (mode == ReadOptions::SEEK_PREVIOUS_SYNC) {
                                    if ((lastDiff > 0 && diff < 0)) {
                                        ALOGI("I got the best seek time,return now");
                                        break;
                                        ...
                                        bIsKeyFrame = false;
                                        repeat_cnt = 10;
                                        ALOGI("seek to last seek Time:%lld, lastDiff:%lld, diff:%lld", (long long)lastSeekTime, (long long)lastDiff, (long long)diff);
                                        continue;
                                    }
                                }
                            ...
                            ALOGI("ASFSource::read seek ASF Video Seek Drop non-I ts = %lld ms", (long long)(keyTimeUs/1000));
                            (*out)->release();  //seek  to 100ms, is not I ,will seek forward, bot nearest I befroe???
                            (*out) = NULL;
                        }
                        repeat_cnt=0;
            ...
            if (mType == ASF_AUDIO) {
                while (1) {
                    ...
                    if (seekTimeUs - keyTimeUs > 300000) {
                        ALOGI("ASF Audio Seek Drop audio2 ts = %lld", (long long)(keyTimeUs/1000));
                        (*out)->release();
                        (*out) = NULL;
                        continue;
                    } else {
...
status_t ASFSource::assembleAVCSizeNalToFrame(MediaBuffer **out) {
    ...
    //int i = 0;
    ALOGI("ASFSource::read()Video Type = AVC,mWantsNALFragments=false,ASF_SIZE_NAL_TYPE ");
...
status_t ASFSource::assembleMp3Frame(MediaBuffer **out) {
    ...
    if((mBuffer != NULL) && (mBuffer->range_length() < 4)) {
        ALOGI("ASFSource::read() mBuffer size(%zu) < 4 bytes, appending buffer for detecting MP3 header",
    ...
    if (mMP3Header >= 0) {
        ALOGD("[ASF_Read]mMP3Header=0x%u", mMP3Header);
    }
    ...
    ALOGI("[ASF_Read]mp3 start %d header %x frameSize %zu length %d bitrate %d",
    ...
    if (start >= 0 && ret) {
        ...
        if (frameSize + (unsigned)start > length) {
            ALOGI("[ASF_Read]MP3 frameSize(%zu) + start(%d) > length(%d)",frameSize, start, length);
            ...
            while(readret == OK) {
                ...
                if (needSize >= nextBuffer->range_length()) {
                    ...
                    ALOGI("[ASF_Read]next asf frame size(%zu) <= need Size(%d)", nextBuffer->range_length(), needSize);
                } else {
                    memcpy((uint8_t*)tmp->data()+ existSize,
...
status_t ASFSource::assembleMjpegFrame(MediaBuffer **out) {
    ...
    *out = clone;
    ALOGI("ASFSource::read() MJPEG MediaBuffer range_offset=%zu, range_length = %zu",
...
ASFErrorType ASFExtractor::GetNextMediaFrame(MediaBuffer** out, bool& bIsKeyFrame,
    ...
    if (*isSeeking) {
        ...
        ALOGI("set curTrackIndex %d's payload count == 0 when seek", curTrackIndex);
...
// ]
    ...
    while (current_frame_size < replicatedata_size) { //replicatedata_size is a frame size
        ...
        //BUT,if this case happen, the Frame type will check error, we will drop the 1th-not-I frame
            ...
            ALOGI("GetNextMediaFrame failed: cur_payload's ts != previous one, cur_payload's ts=%d, pre_payload's ts=%d", timestamp_dummy, timestamp);
    ...
    *out = buffer;
    ...
    ALOGI("GetNextMediaFrame StreamId=%d, ts=%d, IsKey=%d, replicated_size=%d, (*out)->range_length()=%zu, current_frame_size=%d",
            mTracks.editItemAt(curTrackIndex).mTrackNum, timestamp, bIsKeyFrame, replicatedata_size, (*out)->range_length(), current_frame_size);
    ASF_DEBUG_LOGV("mCurPayloadIdx = %d, payload_count = %d\n",
            mTracks.editItemAt(curTrackIndex).mCurPayloadIdx,
...
ASFExtractor::ASFExtractor(const sp<DataSource> &source)
    ...
    ALOGI("ASFExtractor: mFileSize=%lld",(long long)mFileSize);

    ALOGI("+ASFExtractor 0x%p, tid=%d\n", this, gettid());
...
ASFExtractor::~ASFExtractor() {
    ALOGI("~ASFExtractor 0x%p, tid=%d", this, gettid());
...
size_t ASFExtractor::countTracks() {
    ...
    ALOGI("ASFExtractor::countTracks return %zu", mTracks.size());
    return mTracks.size();;
}

sp<MetaData> ASFExtractor::getTrackMetaData(size_t index, uint32_t flags) {
    ALOGI("[ASF]ASFExtractor::getTrackMetaData:mIsAsfParsed=%d,index=%zu\n",mIsAsfParsed,index);
...
sp<MediaSource> ASFExtractor::getTrack(size_t index) {
    ALOGI("getTrack:indx=%zu,mTracks.size()=%zu",index,mTracks.size());
    if (index >= mTracks.size()) {
        return NULL;
    }
...
bool ASFExtractor::ParserVC1CodecPrivateData(uint8_t*input,
    ...
    ALOGI("-----[ASF]CodecPrivateData is 0x%u-----\n",reserved);
...
bool ASFExtractor::RetrieveWmvCodecSpecificData(asf_stream_t * pStreamProp,
        const sp<MetaData>& meta,VC1SeqData* pCodecSpecificData, uint32_t currIdx) {
    bool ret=true;
    char _four_cc[5];
    ...
    // no codec specific data, construct a new one from bitstream
        ...
        if (_config_size_extra_data != 0) {
            ...
            switch (getNALParserType()) {
                ...
                case ASF_START_CODE_TYPE:
                    if (asfMakeAVCCodecSpecificData(csd, meta) != OK) {
                        ALOGI("asfMakeAVCCodecSpecificData fail!!");
                        mIgnoreVideo = true;
                        ret = false;
                    }
                    break;
                case ASF_OTHER_TYPE:
                    ALOGI("asfMakeAVCCodecSpecificData fail for ASF_OTHER_TYPE!!");
                    mIgnoreVideo = true;
                    ret = false;
                    break;
                default:
            ...
            if ( OK != retStatus ) {
                ALOGI("Can not find AVC codec specific data,error = %d\n",ret);
            }
        }
    } else {
...
bool ASFExtractor::RetrieveWmaCodecSpecificData(
    ...
    if(pStreamProp->flags & ASF_STREAM_FLAG_AVAILABLE) {
        ...
        switch (wfx->wFormatTag) {
            ...
            {
                ALOGE(
                     "[VC-1 Playback capability Error] capability not support as:Unknown audio format MSADPCM=0x%hu\n",
                     wfx->wFormatTag);
                return false;;
            }
            ...
            {
                ALOGE(
                     "[VC-1 Playback capability Error] capability not support as:Unknown audio format=0x%hu\n",
                     wfx->wFormatTag);
                return false;
            }
        }
        ALOGI("-----[ASF]RetrieveWmaCodecSpecificData -----\n");
        ALOGI("wfx->wFormatTag = 0x%hu\n", wfx->wFormatTag);
            ...
            if (stream_type == ASF_STREAM_TYPE_AUDIO) {
                ...
                if (!(pStreamProp->flags & ASF_STREAM_FLAG_AVAILABLE)) {
                    ...
                    continue;
                        ...
                        (VC1SeqData*)pCodecSpecificData, i)) {
...
int64_t ASFExtractor::ASFSeekTo(uint32_t targetNPT) {
    int64_t _new_ts = mAsfParser->asf_seek_to_msec(targetNPT); // ms
    ALOGI("[ASF]ASFSeekTo [%d ms] return %lld", targetNPT, (long long)_new_ts);
    return _new_ts;
}
...
status_t ASFExtractor::addMPEG4CodecSpecificData(const sp<MetaData>& mMeta) {
    ...
    if (!foundVolStart) {
        ALOGI("VOL start code: 0x00000120-2F not found!");
    ...
    if (!foundVopStart) {
        ALOGI("VOP start code:0x000001B6 not found!");
        return ERROR_MALFORMED;
    }
    ALOGI("volStart = %zu, vopStart = %zu", volStart, vopStart);
    for (uint32_t i = volStart ; i < vopStart; i++) {
        ALOGI("VOS[%d] = 0x%hhu", i, *((uint8_t *)buffer->data() + i));
    }
...
status_t ASFExtractor::addAVCCodecSpecificData(const sp<MetaData>& mMeta) {
   ...
   // int32_t width = 0;
    ...
    ALOGI("asfDataPos = %lld, read %zx byte\n",(long long)asfDataPos, n);
    if (n < (ssize_t)size) {
    ...
    ALOGI("AVCPos = %d, ptr = 0x%p, offsetStart = %d, prefixLen = %d\n",
          iSPSPos, bufstart, offsetStart, prefixLen);

    if (IsFindPPS == true) {
        if (asfMakeAVCCodecSpecificData(buffer, mMeta) != OK) {
            ALOGI("Make AVCCodec SpecificData fail!!");
            mIgnoreVideo = true;
            return false;
        }
    } else {
        ALOGI(" Error,IsFindPPS == false!!");
        mIgnoreVideo = true;
        return false;
    }
    return OK;
