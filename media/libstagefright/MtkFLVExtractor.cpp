...
FLV_ERROR_TYPE flvParser::flv_read_a_tag(flv_tag_str *tag)
{
    FLV_ERROR_TYPE ret;
    uint8_t* ptr=NULL;
    FLV_DEBUG_LOGV("flv_read_a_tag IN: file offset=0x%p\n",mfile->cur_file_offset);
...
READ_TAG_HEADER:
    ...
    FLV_DEBUG_LOGV("flv_read_a_tag OUT: file offset=0x%p\n",mfile->cur_file_offset);
    return FLV_OK;
}
...
void flvParser::flv_dump_seektable()
{
    ...
    for(i =0;i< mSeekTable->SetEntries;i++)
        FLV_DEBUG_LOGD("--- entry %d ts = %llu offset = %llu ---",
        ...
        if(i < mSeekTable->SetEntries){
            ...
            if(mSeekTable->SetEntries == mSeekTable->MaxEntries){
                ...
                if ((mSeekTable->SetEntries != 0)){
                    ...
                    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                    ts = pEntry[mSeekTable->SetEntries-1].ulTime;
                    FLV_DEBUG_LOGD("flv_seek_to_msec: jump to offset = 0x%llx first",
                            (unsigned long long)mfile->cur_file_offset);
                }else{
                    mfile->cur_file_offset = mfile->data_tag_position;
                    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                ...
                while(1){
                    ...
                    if (ret == FLV_FILE_EOF) {
                        ...
                        ts = pEntry[mSeekTable->SetEntries-1].ulTime;
                        if (ts >= (unsigned)msec) {
                            FLV_DEBUG_LOGD("flv_seek_to_msec: find the tag: ts = %llu, entry = %d",
                ...
                if(mSeekTable->SetEntries==0 && ret!=FLV_OK){
                    ...
                    if(i < mSeekTable->SetEntries){
                        ...
                        mfile->cur_file_offset = pEntry[i].ulOffset; //tag start offset
                        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        FLV_DEBUG_LOGD("flv_seek_to_msec: return 7 %d newTs = %lld, offset = 0x%llx",
                                i, (long long)newTs, (unsigned long long)mfile->cur_file_offset);
                    }else{
                        newTs =  pEntry[mSeekTable->SetEntries-1].ulTime;
                        mfile->cur_file_offset = pEntry[mSeekTable->SetEntries-1].ulOffset; //tag start offset
                        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        FLV_DEBUG_LOGD("flv_seek_to_msec: return 8 entry %d newTs = %lld, offset = 0x%llx",
                                i, (long long)newTs, (unsigned long long)mfile->cur_file_offset);
                    }
                }
...
FLV_ERROR_TYPE flvParser::flv_direct_seek_to_msec(int64_t Trgmsec, int64_t Currmsec, int64_t *Retmsec)
{
    ...
    while(1){
        ...
        FLV_DEBUG_LOGD("flv_direct_seek_to_msec : jump to 0x%llx/0x%llx/0x%llx", file_offset, beh_offset, fwd_offset);
            while(1){
        ...
        if(read_size < 0)
        {
            ...
            if (buf_offset < 0) {
                ALOGD("flv_search_tag_pattern error detected, stop");
                return FLV_FILE_EOF;
            }
    ...
    if(tag_data){
        FLV_DEBUG_LOGM("[memory]flv_direct_seek_to_msec free 0x%p\n",tag_data);
        free(tag_data);
        tag_data=NULL;
    }
    FLV_DEBUG_LOGD("flv_direct_seek_to_msec : final offset = 0x%llx", mfile->cur_file_offset);
    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
...
int32_t flvParser::flv_search_tag_pattern(uint8_t **data, uint32_t size)
{
    ...
    FLV_DEBUG_LOGD("flv_search_tag_pattern : find offset = %u", offset);
    return offset;
}
#endif
...
uint32_t flvParser::flv_search_video_tag_pattern(uint8_t *data, uint32_t size)
{
        ...
        if((zerocnt >= 3) &&
            ...
            if ((i >= 15) && ((start[i - 11]&0x1F) == 0x09)){
                ...
                if((i + tag_data_size + FLV_TAG_HEADER_SIZE) < size){
                    ...
                    //FLV_DEBUG_LOGD("tag_data_size 0x%p, prv_tag_size 0x%p\n", tag_data_size, prv_tag_size);
    ...
    FLV_DEBUG_LOGV("flv_search_video_tag_pattern : find offset = 0x%p\n",offset);
    return offset;
}
...
uint32_t flvParser::flv_search_audio_tag_pattern(uint8_t *data, uint32_t size)
{
    ...
    FLV_DEBUG_LOGV("flv_search_audio_tag_pattern : find offset = 0x%p\n",offset);
    return offset;
}
...
void flvParser::flv_set_file_size(uint64_t file_size)
{
    ...
    if(mfile && ((0 == mfile->file_size) || (file_size != mfile->file_size))){
          if (file_size != mfile->file_size){
             ...
             int32_t read_size = 0;
             FLV_DEBUG_LOGD("flv_set_file_size: file size mismatch, meta/real 0x%llx / 0x%llx",
             ...
             if(tag_data){
                 FLV_DEBUG_LOGM("[memory]flv_set_file_size free 0x%p\n",tag_data);
                 free(tag_data);
                 tag_data=NULL;
             }
          }
        ...
        if(mExtractor->mAudioFrames.size() >= FLV_CACHE_POOL_HIGH) {     // dequeue
            *out = mExtractor->DequeueAudioFrame(mExtractor->mtargetSampleTimeUs);
            ...
            if(FLV_FILE_EOF == retVal){
                FLV_LOGE("FLVSource read: AUDIO CacheMore EOS,BUT still queued %zu frames \n",
...
READ_AGAIN:
#endif
    if (VIDEO == mType) {
        FLV_LOGD("FLVSource::read mVideoFrames.size() = %zu", mExtractor->mVideoFrames.size());
    if (mExtractor->mVideoFrames.size() >= FLV_CACHE_POOL_HIGH) { // dequeue a video frame directly
        ...
        FLV_LOGV("FLVSource read VIDEO buffer1 = 0x%p, ts = %lld\n", *out, (cur_ts/1000));
#if defined(ENABLE_PERF_JUMP_KEY_MECHANISM)
            if (bJumpToKey == true) {
                int32_t keyFrame;
...
size_t FLVExtractor::countTracks()
{
 ...
 bool FLVExtractor::ParseFLV()
 {
     ...
     FLV_LOGD("=====================================\n");
     FLV_LOGD("Resolution = (8,8) - (1280x720)");
...
uint8_t* FLVExtractor::cutAVCC2Buffer(uint8_t *data, size_t size, int buffer_size)
{
 ...
 READ_TAG:
     ...
     if (retVal == FLV_OK)
     {
         ...
         {
             byteVal1 = *(mTag->tag_data);
             FLV_LOGE("CacheMore not video && not audio tag: skip it, byteVal1 = %hhu", byteVal1);
         }
     ...
     {
         FLV_LOGE ("FLVExtractor::CacheMore FLV_FILE_EOF FLV_END_OF_TRACK");
         Ret = FLV_FILE_EOF;
     }
     ...
     switch(byteVal & 0x0F)
     {
         ...
         case FLV_VIDEO_CODEC_ID_AVC:
         {
             ...
             if (mWantsNALFragments == false) {
                 ...
                 MediaBuffer *tempBuffer = new MediaBuffer(pTag->tag_header.tag_data_size);
                 ...
                 while (pTag->tag_data_offset < (pTag->tag_header.tag_data_size - mNALLengthSize)) {
                     ...
                     if (this_nal_size > remain_date_in_tag) {
                         FLV_LOGE("DequeueVideoFrame: current nal size =%d, remain_date_in_tag=%d",this_nal_size,remain_date_in_tag);
                         this_nal_size = remain_date_in_tag;
                     }
                     memcpy(data + full_frame_size, startcode, 3);
                     full_frame_size += 3;
 ...
 void FLVExtractor::ClearVideoFrameQueue()
 {
     ...
     for (size_t i = 0; i < mVideoFrames.size(); i++)
     {
         ...
         if(pTag->tag_data)
         {
             FLV_LOGM("[memory]ClearVideoFrameQueue flush %d mVideoFrames Free buffer =0x%p\n",i,pTag->tag_data);
             free(pTag->tag_data);
             pTag->tag_data = NULL;
         }
     }
     ...
     mVideoFrames.clear();
     FLV_LOGD("ClearVideoFrameQueue clear done,mVideoFrames.size()=%zu\n",mVideoFrames.size());
     ...
     for (size_t i = 0; i < mVideoConfigs.size(); i++)
     {
         ...
         if(pTag->tag_data)
         {
             FLV_LOGM("[memory]ClearmVideoConfigsQueue flush %d mVideoConfigs Free buffer =0x%p\n",i,pTag->tag_data);
             free(pTag->tag_data);
             pTag->tag_data = NULL;
         }
     }
     ...
     mVideoConfigs.clear();
     FLV_LOGD("ClearmVideoConfigsQueue clear done,mVideoConfigs.size()=%zu\n",mVideoConfigs.size());
 }
 void FLVExtractor::ClearAudioFrameQueue()
 {
     ...
     for (size_t i = 0; i < mAudioFrames.size(); i++)
     {
         ...
         if(pTag->tag_data)
         {
             FLV_LOGM("[memory]ClearAudioFrameQueue flush %d mAudioFrames Free buffer =0x%p\n",i,pTag->tag_data);
             free(pTag->tag_data);
             pTag->tag_data = NULL;
         }
     }
     ...
     mAudioFrames.clear();
     FLV_LOGD("-ClearAudioFrameQueue clear done,mAudioFrames.size()=%zu\n",mAudioFrames.size());
     ...
     for (size_t i = 0; i < mAudioConfigs.size(); i++)
     {
         ...
         if(pTag->tag_data)
         {
             FLV_LOGM("[memory]ClearmAudioConfigsQueue flush %d mAudioConfigs Free buffer =0x%p\n",i,pTag->tag_data);
             free(pTag->tag_data);
             pTag->tag_data = NULL;
         }
     }
     ...
     mAudioConfigs.clear();
     FLV_LOGD("ClearmAudioConfigsQueue clear done,mAudioConfigs.size()=%zu\n",mAudioConfigs.size());
 }
