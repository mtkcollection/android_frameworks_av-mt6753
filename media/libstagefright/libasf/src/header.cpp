...
//add by qian
int ASFParser::asf_parse_index_parameters(asf_obj_index_parameters_t *index_parameters,
        uint8_t *objdata, uint32_t objsize) {
        ...
        while (current) {
            ...
            switch (current->type) {
                ...
                case GUID_INDEX_PARAMETERS_OBJECT:
                {
                    ...
                    ALOGI("parser asf_obj_index_parameters_t=0x%p\n", file->header->index_parameters);
...
*/
    if (header->index_parameters) {
        if (header->index_parameters->index_specifiers_entry) {
            ALOGI("asf_free_header:index_specifiers_entry=0x%p\n",
                 header->index_parameters->index_specifiers_entry);
            free(header->index_parameters->index_specifiers_entry);
        }
        ALOGI("asf_free_header:index_parameters=0x%p\n", header->index_parameters);
    ...
    if (current) {
        ...
        for (i=0; i<ret->extended_count; i++) {
            ...
            if (((uint16_t)position + length)> current->datalen) {
                ALOGE("error in extended object key length = %d, tatal object length = %lld",
                     length, current->datalen);
                continue;
            }
            ...
            if (!(ret->extended[i].size) || ((uint16_t)position + length)>current->datalen) {
                ALOGE("error in extended object value length = %d, tatal object length = %lld",
                     length, (current->datalen));
