    ...
    if (mState == DATAGRAM) {
        ...
        do {
            ...
            if (n < 0) {
                ...
                {
                    mPreUs = mCurUs;
                    ALOGI("ANetworkSession Bandwidth=0.0 (KB/s) at %lld Ms", (long long)mPreUs/1000);
                } else {
                    if (mCurUs-mPreUs>=1000000) {
                        int _ms = (mCurUs-mPreUs)/1000;

                        ALOGI("ANetworkSession Bandwidth=%2.2f (KB/s)",
...
///Add by MTK @{
            ...
            while (mInBuffer.size() >= 4) {
                ALOGD("UIBC remain buffer size:%d", (int)mInBuffer.size());
...
void ANetworkSession::Session::dumpFragmentStats(const Fragment &frag) {
    ...
    if( frag.mIsWFDPacket ) {
        ALOGI("[WFD_P][%s][dummy=%d]ts=%lld ms,in %lld ms,out %lld ms, mLatencyF %lld ms,send cost %lld ms,LatencyT %lld ms",
...
#ifdef MTK_AOSP_ENHANCEMENT
status_t  ANetworkSession::Session::sendWFDRequest(
    ...
    while(!packets.empty()){
        ...
        int32_t isVideo = 0;
         int32_t isDummy = 0;
        packet->meta()->findInt32("isLast", &isLastPacket);
        packet->meta()->findInt32("isFirst", &isFirstPacket);
        packet->meta()->findInt32("isVideo", &isVideo);
        packet->meta()->findInt64("latencyB", &latencyB);
        packet->meta()->findInt64("LatencyToken", &LatencyToken);
        packet->meta()->findInt32("isDummy", &isDummy);
...
status_t ANetworkSession::MakeSocketBlocking(int s) {
    int flags = fcntl(s, F_GETFL, 0);
    if (flags < 0) {
        flags = 0;
    }
...
#ifdef MTK_AOSP_ENHANCEMENT
status_t ANetworkSession::sendWFDRequest(
...
status_t ANetworkSession::mtkRTPRecvPause(int32_t sessionID)
{
    Mutex::Autolock autoLock(mLock);
...
status_t ANetworkSession::mtkRTPRecvResume(int32_t sessionID)
{
...
int64_t ANetworkSession::getRTPRecvNum(int32_t sessionID){
...
status_t ANetworkSession::resetRTPRecvNum(int32_t sessionID){
...
///@}
status_t ANetworkSession::setNetworkSessionTestMode()
{
...
status_t ANetworkSession::Session::mtkRTPRecvPause(){
    ALOGD("RTP Receive Pause");
    mRTPPause = true;
    return OK;
}

status_t ANetworkSession::Session::mtkRTPRecvResume(){
    ALOGD("RTP Receive Resume");
    mRTPPause = false;
    return OK;
}

int64_t ANetworkSession::Session::getRTPRecvNum(){

    ALOGD("Receive RTP Number is %lld",(long long)mRTPCounter);
    return mRTPCounter;
}
status_t ANetworkSession::Session::resetRTPRecvNum(){
