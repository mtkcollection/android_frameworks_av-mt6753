...
status_t MtkAVIExtractor::addSample(size_t id, struct MtkAVISample *s) {
    ...
    if (mFileSize < s->offset){//for ALPS01489456,  the last of index table is invalid
        LOGE("addSample--invalid offset,ignore it. s.offset:%d",s->offset );
        return OK;
    }
...
status_t MtkAVIExtractor::parseMOVIMore(bool full) {
    ...
    LOGI("parse movi chunks,mMOVIOffset:0x%lx to setup index from 0x%llx to 0x%llx, mFileSize:0x%lx. full=%d, 0x%lx",mMOVIOffset ,pos, oldEnd, mFileSize ,full,(ssize_t)EVEN(mMOVISize));
    ...
    while(pos < oldEnd) {
        ...
        if (list.ID == BFOURCC('L','I','S','T')) {
            LOGE("type " FORMATFOURCC, PRINTFOURCC(list.type));
            if (list.type == BFOURCC('r', 'e', 'c', ' ')) {
                ...
                while (pos < end) {
                    ...
                    if (s != kSizeOfListHeader && s != kSizeOfChunkHeader) {
                        LOGE("failed to read header at 0x%llx", pos);
                        break;
                    }
            ...
            switch(list.ID) {
                ...
                case BFOURCC('s','t','r','h'):
                    ...
                    if( parseSTRH(pos, end, source)!= OK )//fixed ALPS01487202
                    {
                        ...
                        LOGE("parse strh fail, remove track, %d", mTracks.size());
                    }
                    break;
...
status_t MtkAVIExtractor::parseHDRL(MtkAVIOffT pos, MtkAVIOffT oldEnd, MtkAVIOffT &endOffsCRC) {
    ...
    while(pos < oldEnd) {
        ...
        LOGE("ID " FORMATFOURCC " at pos 0x%llx, list.size:0x%lx,0x%lx", PRINTFOURCC(list.ID), pos,list.size,EVEN(list.size));
            ...
            switch (list.type) {
                ...
                default:
                    ...
                    if (list.type == BFOURCC('m','o','v','i')){
                        ...
                        endOffsCRC = pos;
                        LOGE("parseHDRL, needn't calculate movi, because its list size is very large");
            }
                    break;
            }
        } else {
        ...
        if (err != OK)
            return err;
        if (offsCRC){
            LOGE("some HDRL's param error, break it.");
            break;
        }
        pos = end;
        LOGE("pos 0x%llx", pos);
    ...
    if (pos > oldEnd || !avihFound || i < mNumTracks) {
        LOGE("hdrl seems to be corrupted: end at 0x%llx, expected end at 0x%llx,"
                "avih=%d, actual streams %d, expected streams %d, endOffsCRC:0x%llx",
                pos, oldEnd, avihFound, i, mNumTracks, endOffsCRC);
        return ERROR_MALFORMED;
    }
    return OK;
}
...
status_t MtkAVIExtractor::checkCapability() {
    for(size_t i=0; i<mTracks.size(); ++i) {
        ...
        source->getFormat()->setInt32(kKeyMaxInputSize, source->mMaxSampleSize);
            LOGI("checkCapability sample size %d", source->mMaxSampleSize);
        }
        status_t err = source->generateCodecData(mHasIndex);
        #ifndef ANDROID_DEFAULT_CODE
        if (source->getFormat() == NULL){
            LOGE("checkCapability source->getFormat is NULL ");
            return ERROR_UNSUPPORTED;
        }
        #endif
        if (source->mIsVideo) {
...
status_t MtkAVIExtractor::parseFirstRIFF() {
    ...
    if (size > kAVIMaxRIFFSize) {
        LOGI("size of RIFF is out of spec %ld > %ld", size, kAVIMaxRIFFSize);
        //return ERROR_MALFORMED;
    }
    ...
    int32_t times=0;
    LOGI("size of RIFF  0x%lx -- 0x%lx, pos:0x%llx, oldEnd:0x%llx", size, kAVIMaxRIFFSize, pos, oldEnd);
    while(pos < oldEnd) {
        if (mStopped) {
            LOGI("stopped by user");
            break;
        ...
        if (pos > fSize) {
            LOGW("chunked file? %llx > %llx", pos, fSize);
            break;
        }
        ...
        LOGE("parseFirstRIFF ID " FORMATFOURCC " at pos 0x%llx,list.size:0x%x,0x%x", PRINTFOURCC(list.ID), pos, list.size, EVEN(list.size));
        if ( 0x0 == list.ID ){
        ...
        if (list.ID == BFOURCC('L','I','S','T')) {
            ...
            switch (list.type) {
                case BFOURCC('h','d','r','l'):
                    ...
                    LOGE("found hdrl over,end:0x%llx,endOffsCRC:0x%llx",end ,endOffsCRC);
            ...
            switch (list.ID) {
                ...
                case BFOURCC('i','d','x','1'):
                    LOGE("found idx, idx1Found:%d",idx1Found);
                    if (idx1Found) {
...
status_t MtkAVISource::findRealKeyframe() {
    ...
    int AVCPos = 0;
    int offsetStart = 0;
    LOGW("findRealKeyframe mSampleSizes.size:%d",mSampleSizes.size());
    while(i < mSampleSizes.size() && (i <= 10))
    {
        ...
        LOGW("findRealKeyframe ,i:%d ,size:%d ,offset:%d ",i,size,offset);
        sp<ABuffer> buffer = new ABuffer(size);
        if (mDataSource->readAt(offset, (uint8_t*)buffer->data(), size) != size) {
            LOGE("failed to read data at 0x%08x", offset);
            return ERROR_IO;
        ...
        if (!found) {
            ...
            if (mIsAVC){
                ...
                LOGW("strf size of %d:, AVCPos: %d, dumping first %d", (strfData->size() - AVCPos), AVCPos,s);
            }

             if(mIsAVC)
             {
                mLevel = FindAVCSPS(strfData->data(), strfData->size());
                LOGE("FindAVCSPS, mLevel:%lx",mLevel);
                if(/*0xff == mLevel && */!makeSpecData){
                    i++;
                }else{
                    LOGE("find the real I Frame on the chunk, i:%d", i);
                    break;
                }
             }
        }
    ...
    if(makeSpecData){
        LOGE(" findRealKeyframe-- make specific data");
        return OK;
    }
        ...
        if (mSampleSizes.size() > 0) {
            ...
            if (offsetStart == -1) {
                ...
                if (!found || mCompression == BFOURCC('X', 'V', 'I', 'D')
                    ...
                    #ifndef ANDROID_DEFAULT_CODE
                        if(mFormat == NULL){
                            LOGE("can not MakeAVCCodecSpecificData, need loop for search next chunk");
                            status_t err = findRealKeyframe();
                            if(err != OK){
                                LOGE("find the next frame for making special data fail");
                            }
                        }
                    #endif
                    } else {
