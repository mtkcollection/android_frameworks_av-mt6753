...
status_t ACodec::freeBuffersOnPort(OMX_U32 portIndex) {
    status_t err = OK;
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGD("freeBuffersOnPort portIndex ++ %d", portIndex);
#endif
      ...
      /*
            ...
            if (mOMXLivesLocally) {
                ...
                status_t err = mOMX->setParameter(mNode, OMX_IndexVendorMtkOmxVideoSetClientLocally,
                        &bIsLocally, sizeof(bIsLocally));
                if (err != OK){
                    ALOGD("Failed to set OMX_IndexVendorMtkOmxVideoSetClientLocally(%d)", (int)bIsLocally);
                }
            }
            else {
    ...
    switch (def.eDomain) {
        case OMX_PortDomainVideo:
        {
            ...
            if( IsMTKVideoDecoderComponent(this->mComponentName.c_str()) )
            {
                ...
                OMX_BOOL bIsComponentDoColorConvert = OMX_FALSE;
                if (mOMX->getConfig(
                            mNode, OMX_IndexVendorMtkOmxVdecComponentColorConvert,
                            &bIsComponentDoColorConvert, sizeof(bIsComponentDoColorConvert)) != OK) {
                ...
                if( OMX_COLOR_FormatYUV420Planar != videoDef->eColorFormat )
                {
                    ...
                    if( OMX_TRUE == bIsComponentDoColorConvert )
                    {
...
bool ACodec::LoadedState::onConfigureComponent(
...
#ifdef MTK_AOSP_ENHANCEMENT
    {
        ...
        if (err == OK)
        {
            ALOGD("Found component handle %u", handle);
        }
    }
    ...
    if (IsMTKVideoDecoderComponent(mCodec->mComponentName.c_str())) {
        ...
        int32_t mPropFlags = 0;
        if (msg->findInt32("enableThumbnailOptimzation", &enableThumbnailOptimzation) && (enableThumbnailOptimzation == 1)) {
            err = mCodec->mOMX->setParameter(mCodec->mNode, OMX_IndexVendorMtkOmxVdecThumbnailMode, &mPropFlags, sizeof(void *));
            if (err != OK) {
...
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGD("changeStateIfWeOwnAllBuffers sendCommand +++");
#endif
        status_t err = mCodec->mOMX->sendCommand(
