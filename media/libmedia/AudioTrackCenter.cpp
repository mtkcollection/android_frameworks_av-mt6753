...
status_t AudioTrackCenter::addTrack(intptr_t trackId, uint32_t frameCount, uint32_t sampleRate, void* trackPtr,  uint32_t afFrameCount, uint32_t afSampleRate, uint32_t framesFilled) {
    ...
    mAfSampleRate = afSampleRate;
         mAfFrameCount = afFrameCount;
            SLOGD("mAfSampleRate %d, sampleRate %d, AfFrameCount %d , mAfSampleRate %d, frameCount %d",mAfSampleRate, sampleRate, mAfFrameCount, mAfSampleRate, frameCount  );
...
status_t AudioTrackCenter::updateServer(intptr_t trackId, uint32_t server, bool restore) {
    ...
    SLOGV("%s, trackId:%p, server:%d",__FUNCTION__, (void*)trackId, server);
    #endif
    ssize_t index = mTrackList.indexOfKey(trackId);
    if (index < 0) {
        return UNKNOWN_ERROR;
    ...
    info.server = server;
#if defined(CONFIG_MT_ENG_BUILD)
    SLOGV("trackId:%p, info.server:%d, info.framePlayed:%lld, info.ts:%lld",(void*)trackId, info.server, info.framePlayed, info.ts);
#endif
    info.middleServer  = server;
...
#endif
    #if defined(CONFIG_MT_ENG_BUILD)
    ALOGD("trackId = %p, server = %d, framePlayed = %lld, delayUs = %lld, *position:%lld", (void*)trackId, info.server, info.framePlayed, delayUs, *position);
    #endif
    return OK;
}
